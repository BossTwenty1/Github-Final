...
export function PlotManagementCrud() {
  ...
  const [areas, setAreas] = useState<PlotAreaOption[]>([]);
  ...
  const [page, setPage] = useState(0);
  const [hasMore, setHasMore] = useState(false);
  const refresh = useCallback(async () => { try { const [lotPage, nextOwners, nextAreas] = await Promise.all([getAdminLotsPage({ page: 0 }), getOwnerRecords({ page: 0, pageSize: 100 }), getPlotAreas()]); setLots(lotPage.items); setPage(0); setHasMore(lotPage.hasMore); setOwners(nextOwners); setAreas(nextAreas); setError(""); } catch (reason) { setError(errorMessage(reason)); } }, []);
  useEffect(() => { const timer = window.setTimeout(() => { void refresh(); }, 0); return () => window.clearTimeout(timer); }, [refresh]);
  async function loadMore() { if (loadingMore || !hasMore) return; setLoadingMore(true); try { const nextPage = await getAdminLotsPage({ page: page + 1 }); setLots((current) => [...current, ...nextPage.items]); setPage((current) => current + 1); setHasMore(nextPage.hasMore); } catch (reason) { setError(errorMessage(reason)); } finally { setLoadingMore(false); } }
  const available = lots.filter((lot) => lot.status === "AVAILABLE").length;
  const booked = lots.filter((lot) => lot.status === "BOOKED").length;
  const areaLabels = new Map(areas.map((area) => [area.areaId, `${area.label} (${area.areaCode})`]));
  ...
  const searchOwners = useCallback(async (search: string) => (await getOwnerRecordsPage({ search, pageSize: 20 })).items, []);
  async function removeLot() { if (!deleteTarget || busy) return; setBusy(true); try { await deleteLot(deleteTarget.lotId, deleteTarget.revision); setDeleteTarget(null); setMessage("Plot removed. It can be restored in Recovery."); await refresh(); } catch (reason) { setError(errorMessage(reason)); } finally { setBusy(false); } }
  return <AdminPageFrame title="Plot Management" description="Create, edit, and remove plot records while preserving existing coordinate data." actions={<Button icon="plus" onClick={() => { setEditing(null); setFormOpen(true); }}>Add plot</Button>}>
    ...
    {formOpen ? <LotForm key={editing?.lotId ?? "new"} lot={editing} areas={areas} owners={owners} onSearchOwners={searchOwners} onCancel={() => { setFormOpen(false); setEditing(null); }} onSaved={async () => { setFormOpen(false); setEditing(null); setMessage(editing ? "Plot updated." : "Plot created."); await refresh(); }} /> : null}
    <div className="admin-summary-grid"><SummaryCard label="Known plots" value={lots.length} /><SummaryCard label="Available" value={available} /><SummaryCard label="Booked" value={booked} /></div>
     <Card><CardHeader><CardTitle>Local plots</CardTitle><Badge variant="info">{lots.length}{hasMore ? "+" : ""} loaded</Badge></CardHeader>{lots.length ? <><Table caption="Local plots"><TableHead><TableRow><TableHeaderCell>Plot</TableHeaderCell><TableHeaderCell>Garden / area</TableHeaderCell><TableHeaderCell>Owner</TableHeaderCell><TableHeaderCell>Status</TableHeaderCell><TableHeaderCell>Coordinates</TableHeaderCell><TableHeaderCell>Actions</TableHeaderCell></TableRow></TableHead><TableBody>{lots.map((lot) => <TableRow key={lot.lotId}><TableCell className="strong-cell">{lot.lotCode}<span className="table-secondary">{lot.legacyLocationCode || "No original reference"}</span></TableCell><TableCell>{areaLabels.get(lot.areaId) || "Unassigned"}</TableCell><TableCell>{lot.lotOwnerId ? ownerLabels.get(lot.lotOwnerId) || "Unknown owner" : "Unassigned"}</TableCell><TableCell><Badge variant={lot.status === "AVAILABLE" ? "success" : lot.status === "BOOKED" ? "warning" : "neutral"}>{lot.status}</Badge></TableCell><TableCell>{lot.location ? "GPS " + (lot.coordinateVerified ? "verified" : "unverified") : lot.pxLocX !== null && lot.pxLocY !== null ? "Image-map" : "Unavailable"}</TableCell><TableCell><div className="table-actions"><Button size="sm" variant="secondary" onClick={() => { setEditing(lot); setFormOpen(true); }}>Edit</Button><Button disabled={busy || !canAdmin} title={canAdmin ? "Remove record" : "Only administrators can remove records"} size="sm" variant="danger" onClick={() => setDeleteTarget(lot)}>Delete</Button></div></TableCell></TableRow>)}</TableBody></Table>{hasMore ? <CardContent><Button disabled={loadingMore} onClick={() => void loadMore()} variant="secondary">{loadingMore ? "Loading more…" : "Load more plots"}</Button></CardContent> : null}</> : <CardContent><EmptyState description="Add a plot after its garden / area exists in Supabase." icon="grid" title="No plots found" /></CardContent>}</Card>
    <ConfirmationDialog open={Boolean(deleteTarget)} title="Delete plot?" description={<p>Only plots without a burial record can be removed. Administrators can restore removed plots in Recovery.</p>} confirmLabel={busy ? "Deleting…" : "Delete plot"} variant="danger" onConfirm={() => void removeLot()} onClose={() => { if (!busy) setDeleteTarget(null); }} />
  </AdminPageFrame>;
}

function LotForm({ lot, areas, owners, onSearchOwners, onCancel, onSaved }: { lot: AdminLot | null; areas: PlotAreaOption[]; owners: LotOwner[]; onSearchOwners: (search: string) => Promise<LotOwner[]>; onCancel: () => void; onSaved: () => Promise<void> }) {
  const [form, setForm] = useState<LotInput>(() => lot ? lotInputFromLot(lot) : { areaId: 0, lotOwnerId: null, lotCode: "", legacyLocationCode: "", legacyPaNumber: "", status: "AVAILABLE", lengthM: null, widthM: null });
  const [error, setError] = useState("");
  const [saving, setSaving] = useState(false);
  function setField<K extends keyof LotInput>(key: K, value: LotInput[K]) { setForm((current) => ({ ...current, [key]: value })); }
  async function submit(event: FormEvent<HTMLFormElement>) { event.preventDefault(); if (saving) return; setError(""); if (!form.areaId || !form.lotCode.trim()) { setError("Select a garden / area and enter a plot label or code."); return; } if ((form.lengthM !== null && form.lengthM <= 0) || (form.widthM !== null && form.widthM <= 0)) { setError("Plot dimensions must be greater than zero."); return; } const hasLongitude = form.longitude !== null && form.longitude !== undefined && String(form.longitude) !== ""; const hasLatitude = form.latitude !== null && form.latitude !== undefined && String(form.latitude) !== ""; if (hasLongitude !== hasLatitude) { setError("Enter both longitude and latitude, or leave both coordinates blank."); return; } if (hasLongitude && (!Number.isFinite(form.longitude) || !Number.isFinite(form.latitude))) { setError("Enter valid numeric coordinates."); return; } const coordinatesChanged = Boolean(lot && (form.longitude !== (lot.location?.longitude ?? null) || form.latitude !== (lot.location?.latitude ?? null))); setSaving(true); try { const input = { ...form, lotCode: form.lotCode.trim(), legacyLocationCode: form.legacyLocationCode?.trim() || "", coordinateStatus: coordinatesChanged ? "pending" as const : form.coordinateStatus, coordinateVerified: coordinatesChanged ? false : form.coordinateVerified }; await (lot ? updateLot(lot.lotId, input) : createLot(input)); await onSaved(); } catch (reason) { setError(errorMessage(reason)); } finally { setSaving(false); } }
  return <Card className="admin-form-card"><CardHeader><CardTitle>{lot ? "Edit plot" : "Add plot"}</CardTitle><Badge variant="info">Coordinate-safe operation</Badge></CardHeader><CardContent><form className="admin-form" onSubmit={(event) => void submit(event)}><div className="filter-grid"><SelectField id="plot-area" label="Garden / area" value={String(form.areaId || "")} onChange={(value) => setField("areaId", Number(value))} options={[["", "Select a garden / area"], ...areas.map((area) => [String(area.areaId), `${area.label} (${area.areaCode})`] as [string, string])]} hint="Choose one of the four Phase 1 gardens." /><Input id="plot-code" label="Plot label or code" hint="The visible identifier used by administrators." onChange={(event) => setField("lotCode", event.target.value)} required value={form.lotCode} /></div><div className="filter-grid"><Input id="plot-legacy-location" label="Original location reference (optional)" hint="Use the old Lot Location value when this plot has an older record. Leave blank for a new plot." onChange={(event) => setField("legacyLocationCode", event.target.value)} value={form.legacyLocationCode || ""} /><Input id="plot-legacy-pa" label="Old PA number (optional)" hint="Leave blank if no confirmed value exists." onChange={(event) => setField("legacyPaNumber", event.target.value)} value={form.legacyPaNumber || ""} /></div><div className="filter-grid"><OwnerPicker owners={owners} onSearchOwners={onSearchOwners} value={form.lotOwnerId ? String(form.lotOwnerId) : ""} onChange={(value) => setField("lotOwnerId", value ? Number(value) : null)} /><SelectField id="plot-status" label="Plot availability" value={form.status} onChange={(value) => setField("status", value as LotInput["status"])} options={[["AVAILABLE", "Available for a new burial"], ["BOOKED", "Reserved or assigned"], ["HOLD", "Temporarily unavailable"]]} /></div><div className="filter-grid"><Input id="plot-length" label="Plot length (m)" min="0" onChange={(event) => setField("lengthM", optionalNumber(event.target.value))} step="0.01" type="number" value={form.lengthM ?? ""} /><Input id="plot-width" label="Plot width (m)" min="0" onChange={(event) => setField("widthM", optionalNumber(event.target.value))} step="0.01" type="number" value={form.widthM ?? ""} /></div><div className="filter-grid"><Input id="plot-longitude" label="Longitude (optional)" hint="Paste the longitude recorded in Google Earth or on site." onChange={(event) => setField("longitude", optionalNumber(event.target.value))} step="any" type="number" value={form.longitude ?? ""} /><Input id="plot-latitude" label="Latitude (optional)" hint="Paste the latitude recorded in Google Earth or on site." onChange={(event) => setField("latitude", optionalNumber(event.target.value))} step="any" type="number" value={form.latitude ?? ""} /></div><Input id="plot-coordinate-accuracy" label="Coordinate accuracy in meters (optional)" min="0" onChange={(event) => setField("coordinateAccuracyM", optionalNumber(event.target.value))} step="0.01" type="number" value={form.coordinateAccuracyM ?? ""} /><Alert icon="info" title="Coordinate verification">New or changed coordinates are saved as pending until an administrator verifies them in Coordinate Verification.</Alert>{error ? <DataError message={error} /> : null}<div className="admin-form-actions"><Button onClick={onCancel} type="button" variant="secondary">Cancel</Button><Button disabled={saving || !areas.length || !form.areaId} icon="check" type="submit">{saving ? "Saving…" : lot ? "Save changes" : "Create plot"}</Button></div></form></CardContent></Card>;
}
...
export function OwnerDataCrud() {
  ...
  const [page, setPage] = useState(0);
  const [hasMore, setHasMore] = useState(false);
  const refresh = useCallback(async () => { try { const ownerPage = await getOwnerRecordsPage({ page: 0 }); setOwners(ownerPage.items); setPage(0); setHasMore(ownerPage.hasMore); setError(""); } catch (reason) { setError(errorMessage(reason)); } }, []);
  useEffect(() => { const timer = window.setTimeout(() => { void refresh(); }, 0); return () => window.clearTimeout(timer); }, [refresh]);
  async function loadMore() { if (loadingMore || !hasMore) return; setLoadingMore(true); try { const nextPage = await getOwnerRecordsPage({ page: page + 1 }); setOwners((current) => [...current, ...nextPage.items]); setPage((current) => current + 1); setHasMore(nextPage.hasMore); } catch (reason) { setError(errorMessage(reason)); } finally { setLoadingMore(false); } }
  async function removeOwner() { if (!deleteTarget || busy) return; setBusy(true); try { await deleteLotOwner(deleteTarget.lotOwnerId, deleteTarget.revision); setDeleteTarget(null); setMessage("Owner removed. It can be restored in Recovery."); await refresh(); } catch (reason) { setError(errorMessage(reason)); } finally { setBusy(false); } }
  return <AdminPageFrame title="Owner data" description="Create, edit, and remove protected lot-owner records used by cemetery operations." actions={<Button icon="plus" onClick={() => { setEditing(null); setFormOpen(true); }}>Add owner</Button>}>
    ...
    <Card><CardHeader><CardTitle>Lot owners</CardTitle><Badge variant="info">{owners.length}{hasMore ? "+" : ""} loaded</Badge></CardHeader>{owners.length ? <><Table caption="Approved lot owners"><TableHead><TableRow><TableHeaderCell>Owner</TableHeaderCell><TableHeaderCell>Aliases</TableHeaderCell><TableHeaderCell>Address</TableHeaderCell><TableHeaderCell>Representative</TableHeaderCell><TableHeaderCell>Actions</TableHeaderCell></TableRow></TableHead><TableBody>{owners.map((owner) => <TableRow key={owner.lotOwnerId}><TableCell><strong>{[owner.firstName, owner.middleName, owner.lastName, owner.suffix].filter(Boolean).join(" ")}</strong></TableCell><TableCell>{owner.aliases || "None recorded"}</TableCell><TableCell>{owner.address}</TableCell><TableCell>{owner.representativeName ? owner.representativeName + (owner.representativeContact ? " · " + owner.representativeContact : "") : "None recorded"}</TableCell><TableCell><div className="table-actions"><Button size="sm" variant="secondary" onClick={() => { setEditing(owner); setFormOpen(true); }}>Edit</Button><Button disabled={busy || !canAdmin} title={canAdmin ? "Remove record" : "Only administrators can remove records"} size="sm" variant="danger" onClick={() => setDeleteTarget(owner)}>Delete</Button></div></TableCell></TableRow>)}</TableBody></Table>{hasMore ? <CardContent><Button disabled={loadingMore} onClick={() => void loadMore()} variant="secondary">{loadingMore ? "Loading more…" : "Load more owners"}</Button></CardContent> : null}</> : <CardContent><EmptyState description="Add owner data when an authorized administrator is ready to manage it." icon="userSearch" title="No owner data" /></CardContent>}</Card>
    <ConfirmationDialog open={Boolean(deleteTarget)} title="Delete owner?" description={<p>Only unassigned owners can be removed. Administrators can restore them in Recovery.</p>} confirmLabel={busy ? "Deleting…" : "Delete owner"} variant="danger" onConfirm={() => void removeOwner()} onClose={() => { if (!busy) setDeleteTarget(null); }} />
  </AdminPageFrame>;
}
...
function OwnerForm({ owner, onCancel, onSaved }: { owner: LotOwner | null; onCancel: () => void; onSaved: () => Promise<void> }) {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) { event.preventDefault(); if (saving) return; setError(""); if (!form.firstName.trim() || !form.lastName.trim() || !form.address.trim()) { setError("First name, last name, and address are required."); return; } setSaving(true); try { const input = { ...form, firstName: form.firstName.trim(), lastName: form.lastName.trim(), address: form.address.trim() }; await (owner ? updateLotOwner(owner.lotOwnerId, input) : createLotOwner(input)); await onSaved(); } catch (reason) { setError(errorMessage(reason)); } finally { setSaving(false); } }
...
function OwnerPicker({ value, owners, onSearchOwners, onChange }: { value: string; owners: LotOwner[]; onSearchOwners: (search: string) => Promise<LotOwner[]>; onChange: (value: string) => void }) {
  const [query, setQuery] = useState("");
  const [remoteMatches, setRemoteMatches] = useState<LotOwner[] | null>(null);
  const normalizedQuery = query.toLowerCase().trim();
  useEffect(() => {
    if (normalizedQuery.length < 2) return;
    let active = true;
    void onSearchOwners(normalizedQuery).then((result) => { if (active) setRemoteMatches(result); }).catch(() => { if (active) setRemoteMatches([]); });
    return () => { active = false; };
  }, [normalizedQuery, onSearchOwners]);
  const matches = normalizedQuery.length >= 2 ? remoteMatches ?? [] : owners.filter((owner) => [owner.firstName, owner.middleName, owner.lastName, owner.suffix, owner.aliases].filter(Boolean).join(" ").toLowerCase().includes(normalizedQuery));
  ...
  return <div className="input-field"><span className="input-label">Plot owner (optional)</span><input aria-controls="plot-owner-results" aria-expanded={hasSearch && matches.length > 0} aria-label="Search plot owners" aria-autocomplete="list" className="input-control" id="plot-owner-search" onChange={(event) => setQuery(event.target.value)} onKeyDown={(event) => { if (event.key === "Enter" && matches[0]) { event.preventDefault(); selectOwner(matches[0]); } }} placeholder="Search owner name or alias" role="combobox" value={query} />{selected ? <span className="input-hint">Selected owner: <strong>{ownerName(selected)}</strong> <button className="text-button" onClick={() => { onChange(""); setQuery(""); }} type="button">Clear</button></span> : null}{hasSearch && matches.length ? <div aria-label="Matching plot owners" className="owner-picker-results" id="plot-owner-results" role="listbox">{matches.slice(0, 20).map((owner) => <button aria-selected={owner.lotOwnerId === selected?.lotOwnerId} className="text-button" key={owner.lotOwnerId} onClick={() => selectOwner(owner)} role="option" type="button">{ownerName(owner)}</button>)}</div> : null}{hasSearch && !matches.length ? <span className="input-hint">No owners found. <Link className="subtle-link" href="/admin/owner-data">Create a new owner in Owner Data.</Link></span> : null}{!owners.length && !hasSearch ? <span className="input-hint">No owners found. <Link className="subtle-link" href="/admin/owner-data">Create a new owner in Owner Data.</Link></span> : null}<span className="input-hint">Type at least 2 characters to search. One owner may have multiple plots.</span></div>;
}
...
function DataError({ message }: { message: string }) { return <Alert icon="alert" title="Change could not be completed" variant="danger">{message}</Alert>; }
